...
\subsubsection{Modality as Alignment}\label{subsec:modality-alignment}
...
The dissociation between subjective load and physiological engagement (detailed in \autoref{subsec:modality-alignment}) extends Cognitive Load Theory. It reframes modality choice as an issue of "schema-format matching," not just a universal design prescription. For STEM learners accustomed to dynamic, causal representations, video may sustain cognitive investment, while for narrative-oriented learners, text's low-transience affordances may be superior.
...
Future research should also refine the process measures used in this thesis. First, co-registering pupil trajectories with spatial scanpath features would test whether physiological relief coincides with tighter spatial regularity or more economical forward advancement under matched pacing. Second, explicitly manipulating segmentation granularity and signaling would probe whether the early-section scaffolding observed in Study 5 is indeed the locus of the video advantage. Third, embedding comprehension probes aligned to section boundaries would help clarify when lower invested effort reflects genuine coordination relief rather than disengagement.
