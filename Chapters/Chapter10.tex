%************************************************
\chapter{Study 5: Temporal Dynamics of Cognitive Load}\label{ch:study5}
%************************************************
...
\section{Background}
\subsection{Pupil Dilation and Cognitive Load}
...
The reviewed evidence establishes that pupil dilation tracks processing effort during L2 comprehension, but most studies have examined short stimuli or extracted single summary measures. Building on the temporal dynamics framework established in \autoref{subsec:extraneous}, the present study extends this work to academic passages and models the complete temporal trajectory to reveal when and how cognitive load are distributed over time.
...
Models were specified as follows. For trend (the primary outcome), we tested fixed effects of complexity, modality, domain, and major, with the single planned interaction modality $\times$ major that operationalizes the learner–format alignment hypothesis:
\[
...
\end{align*}
Modality took levels text and video; major took levels science and arts and social science majors. Given the single planned interaction for trend, multiplicity was handled by limiting the model space a priori and by corroborating the focal effect with a permutation test that shuffled condition labels within-sections; this yielded $p<.002$. For transparency we also report model $R^{2}$ values and unadjusted $p$ values for all terms in the main text and tables.
...
Time-integrated contrasts of text minus video, presented in Table~\ref{tab:modality-contrasts}, revealed a \textit{selective} video advantage. A video advantage, that is, smaller pupil dilation for video than text, was evident for simple history and complex science: pupil size was reduced by a little over half a standard deviation in simple history and by somewhat more than two thirds of a standard deviation in complex science (simple history: estimate $= -0.616$~SD, $p = .008$; complex science: estimate $= -0.721$~SD, $p = .002$). No reliable modality differences were observed for complex history or for simple science (complex history: estimate $= -0.168$~SD, $p = .466$; simple science: estimate $= -0.127$~SD, $p = .580$). Temporal inspection of these contrasts, visualized in Figure~\ref{fig:temporal-dynamics}, showed early onset and sustained differences in the two significant cells, with 95\% to 100\% of time points remaining significant after false discovery rate correction. 

Model diagnostics supported this interpretation. The model explained a substantial proportion of total deviance, indicating good overall fit ($R^2 = 0.479$). Participant-specific temporal smooths captured large between-participant differences in trajectory shape (factor smooth for normalized time: edf $\approx 283.83$, $F \approx 1689$, $p < .001$), and the two-dimensional gaze-contingent smooth likewise contributed meaningfully (edf $\approx 18.67$, $F \approx 63$, $p < .001$). Time-by-condition interaction smooths were significant as a set (all $p < .001$), validating the nonlinear specification. Full coefficient tables and additional diagnostic information are provided in the Appendix.
...
\begin{figure}[h!]
\centering
\includegraphics[width=\linewidth]{Chapters/study5fig/s5f1.pdf} % save your first plot with this name
...
Trajectory-level analyses identified a single effect that was both large and stable: a \textit{modality} $\times$ \textit{major} interaction on trend. Aggregating trajectory-level trend codes within each condition, science majors under video showed a clearly positive mean trend (approximately $+0.833$), whereas the same group under text showed a negative mean trend (approximately $-1.000$). arts and social science majors showed negative or near-zero mean trend in both modalities (approximately $-0.667$ for text and $-0.364$ for video). Thus, video shifted science majors from decreasing to increasing pupil dilation over a section, while non-science majors remained flat or declining regardless of modality. The interaction is visible in the trend proportion plot (Figure~\ref{fig:rq2-trend-patterns}) and in the predicted trajectories (Figure~\ref{fig:rq2-trajectories}).
...
\begin{figure}[ht]
\centering
\includegraphics[width=\linewidth]{Chapters/study5fig/s5_rq2_trend_patterns.pdf} % save your first plot with this name
...
\begin{figure}[htb]
\centering
\includegraphics[width=\linewidth]{Chapters/study5fig/s5_rq2_trajectories.pdf} % save your trajectories panel with this name
\caption[Predicted pupil trajectories]{Predicted pupil trajectories by presentation modality and major, shaded areas show 95\% confidence intervals. Science majors show sustained or rising engagement with video and a declining pattern with text, which aligns with the trend classification used for analysis.}
\label{fig:rq2-trajectories}
\end{figure}


The planned linear models were estimated on $n = 47$ trajectories, focusing on theoretically motivated two-way terms to limit overfitting. The trend model included the single planned \textit{modality} $\times$ \textit{major} interaction, whereas magnitude and shape-complexity models included \textit{complexity} $\times$ \textit{modality} and \textit{domain} $\times$ \textit{major}. Time-structured trajectories were the norm: forty-seven of forty-eight cell-level temporal smooths passed false discovery rate screening, confirming that within-section pupil dynamics were systematically shaped by the design.

In the trend model, the modality-by-major interaction corresponded to an estimated shift of about one and a half units on the trend scale, indicating a substantial change in trajectory direction (estimate $= 1.53$, $t = 4.17$, two-sided $p < .001$), and the model showed strong overall fit ($R^2 = 0.593$). This effect remained stable across threshold choices for the trend classifier: estimates stayed positive and of similar magnitude across bands from one half percent to five percent (1.11–1.53 on the trend scale; all $p\text{s} < .005$). A nonparametric permutation test, shuffling condition labels across trajectories, yielded a similarly low probability of the interaction under the null model (permutation $p < .001$). By contrast, the models for magnitude and complexity explained only modest variance ($R^2 = 0.130$ and $R^2 = 0.149$, respectively), suggesting that amplitude and local volatility of the pupil trajectory were less systematically governed by the experimental factors than temporal direction.
