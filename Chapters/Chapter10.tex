...
Study 3 established that linguistic complexity, domain, and modality can jointly shape cognitive load, self-efficacy, and comprehension accuracy. Study 4 examined how these factors manifest in eye-movement behavior during text reading, revealing that linguistic complexity dominates global timing and spatial attention patterns, with domain expertise producing selective, context-dependent benefits. However, both outcome measures (Study 3, \autoref{ch:study3}) and spatial eye-movement patterns (Study 4, \autoref{ch:study4}) face a common limitation: they cannot capture moment-to-moment fluctuations in cognitive effort within a single passage section. Aggregate measures reduce dynamic processes to static summaries. Comprehension scores, mean fixation durations, and total reading times may miss transient spikes, recoveries, and temporal patterns that reveal how learners allocate limited resources over time.

Study 5 addresses this gap by analyzing continuous pupil diameter trajectories as a time-resolved physiological index of cognitive load. Pupil dilation reliably tracks processing effort during reading, scaling with linguistic complexity, lexical difficulty, and integration demands \parencite{zekveld2010pupil, pichora2016hearing}. Critically, pupillometry provides three advantages over the measures used in Studies 3 and 4. First, it is continuous and sampled at high temporal resolution, capturing within-section dynamics that aggregate indices miss. Second, it operates across both text and video modalities, whereas the position-anchored eye-movement measures in Study 4 required stable spatial layouts and were restricted to text. Third, because the pupil signal is slow and autocorrelated, effects may be time-localized rather than uniform (e.g., early coordination costs during initial integration or late-section fatigue), which motivates modeling the complete temporal trajectory rather than extracting single summary features.

The central question is whether the outcome patterns observed in Study 3 and the eye-movement signatures documented in Study 4 manifest as distinct temporal profiles of physiological effort, and whether these profiles reveal when and how cognitive demands are distributed over time. Specifically, does linguistic complexity produce sustained elevation throughout a section or transient early spikes? Does video presentation reduce effort uniformly or only during specific integration windows when audiovisual coordination is most demanding? Does domain expertise buffer effort by lowering peak demands, accelerating recovery, or both? To address these questions, the present study models pupil diameter trajectories using generalized additive mixed models. These models estimate smooth, potentially nonlinear curves over normalized section time and allow condition-specific temporal patterns to emerge from the data (RQ1). Beyond average levels, we examine whether the temporal shape of trajectories (rise steepness, peak timing, curvature, and decay) varies systematically with task and learner factors (RQ2). Together, these analyses provide a physiological complement to the eye-movement evidence in Study 4, clarifying not only where processing costs are highest but when they accumulate and dissipate within extended L2 academic reading.
...
\caption{Temporal dynamics of modality effects, Text minus Video, across normalized time. Shaded ribbons indicate 95\% confidence intervals. Red marks time points that remain significant after false discovery rate correction.}
...
\caption{Predicted pupil trajectories by presentation modality and major, shaded areas show 95 percent confidence intervals. Science majors show sustained or rising engagement with video and a declining pattern with text, which aligns with the trend classification used for analysis.}
...
Practical implications follow. First, manage intrinsic load to create headroom, for example by controlling linguistic complexity and cohesion, since these govern scanpath organization and baseline effort \parencite{rayner1998,VonderMalsburg2015}. Second, where content structure permits cross channel scaffolding, prefer segmented and signaled video presentations that co time narration with visual change and avoid redundancy \parencite{mayer2003,ginns2006,spanjers2012}. Third, anticipate stronger sustained engagement for STEM trained readers under video presentation in simple narrative history and linguistically complex science, with more heterogeneous patterns for Arts and Social Sciences majors, an alignment consistent with disciplinary literacy \parencite{shanahan2008teaching,wineburg1991reading}. 

Future work can test whether cells showing pupil based relief also exhibit tighter scanpaths or more economical forward advance under matched pacing, co registering trajectory features with spatial regularity and regression patterns \parencite{VonderMalsburg2015,GodfroidWinke2019}. A second direction is to manipulate segmentation granularity and signaling explicitly to probe whether early section scaffolding is the locus of the video advantage \parencite{spanjers2012,alpizar2020}. A third is to add comprehension probes aligned to section boundaries to clarify when lower invested effort reflects genuine coordination relief rather than disengagement \parencite{mayer2003,mattys2012speech}.
