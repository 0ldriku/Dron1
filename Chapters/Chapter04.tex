...
\subsection{Prioritization and Fluctuations in CAF }
...
\begin{figure}[htb]
  ...
  \caption[Homogeneity of dispersion]{Homogeneity of dispersion (PERMDISP): distance to centroid by task complexity. Medians and IQRs overlap across conditions, consistent with the non-significant dispersion test.}
  \label{fig:disp-load}
\end{figure}
...
\caption[PERMANOVA by terms on Euclidean distances of the five indicators]{PERMANOVA by terms on Euclidean distances of the five indicators (999 permutations)}
\footnotesize
\label{tab:permanova}
...
\caption[Linear discriminant coefficients]{Linear discriminant coefficients (LD1). Positive values indicate higher scores on the fluency and phrasal side of the discriminant}
\footnotesize
\label{tab:lda-coef}
...
These findings set up the next steps in the thesis. Subsequent studies examine whether analogous reallocations appear in comprehension. Together, this program moves from establishing allocation under load in production to linking performance profiles with evaluation.