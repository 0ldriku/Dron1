%************************************************
\chapter{Experiment 1 Method (Studies 1 and 2)}\label{ch:exp1-methods}
%************************************************
...
\subsection{Oral Task}
The oral task was adapted from \textcite{michel2011} for a Japanese context. Participants were instructed to assume the role of a director for a romance reality show and determine pairings between male and female contestants. The task featured two versions differing in complexity: a low-complexity task and a high-complexity task. In the low-complexity version, each speaker saw four photographs of young adults on a PC screen (two males and two females; see Figure \ref{fig:task-layout}, left panel); in the high-complexity version, they saw eight (four males and four females; Figure \ref{fig:task-layout}, right panel). Alongside each photograph were four attributes: job, personality, favorite hobby, and ``type'' (in the sense of a romantic partner). Speakers were required to form two male-female couples in the low-complexity task (choosing from four contestants) or four couples in the high-complexity task (from eight). The expected speech duration was 90 seconds for the low-complexity task and 150 seconds for the high-complexity task.



\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth]{Chapters/c3fig/malefemallayout.pdf}
    \caption[Layout of the oral task materials]{Layout of the oral task materials. The left panel shows the low-complexity version with four contestants (two males and two females). The right panel shows the high-complexity version with eight contestants (four males and four females). Each contestant's photograph was accompanied by four attributes: job, personality, favorite hobby, and preferred partner type.}
    \label{fig:task-layout}
\end{figure}
...
\subsection{Linguistic Dimensions}\label{subsec:linguistic-dimensions}
...
\textit{Speed fluency}
\begin{enumerate}
    \item Speech rate: The mean number of moras\footnote{Mora: A beat or unit of sound timing in Japanese. Typically, one hiragana character (like \begin{CJK}{UTF8}{ipxm}か\end{CJK}) equals one mora. However, small characters for palatalized sounds (like \begin{CJK}{UTF8}{ipxm}ゃ\end{CJK} in \begin{CJK}{UTF8}{ipxm}きゃ\end{CJK}) combine with the preceding kana, and these two characters together count as only one mora. The syllabic nasal (\begin{CJK}{UTF8}{ipxm}ん\end{CJK}), the geminate consonant (\begin{CJK}{UTF8}{ipxm}っ\end{CJK}), and the long vowel mark (\begin{CJK}{UTF8}{ipxm}ー\end{CJK}) also each count as one full mora by themselves.} per second, calculated by dividing the total number of moras by the total speech duration.
...
\caption{Perceived difficulty, mental effort, and LISAS scores}
\footnotesize
...
\midrule
\multicolumn{5}{l}{\textit{Low-complexity Task}} \\
...
\midrule
\multicolumn{5}{l}{\textit{High-complexity Task}} \\
...
\subsection{Analytical Roadmap for Studies 1 and 2}


The data collected and processed in Experiment 1 serve as the foundation for the analyses in the following two chapters.

\begin{description}
    \item[Study 1 (\autoref{ch:study1}):] Analyzes the linguistic dimensions calculated from the speaker audio recordings to investigate how L2 production profiles shift under different cognitive load conditions.
    \item[Study 2 (\autoref{ch:study2}):] Uses both the linguistic dimensions and the ratings (comprehensibility and perceived fluency) provided by the native Japanese raters. This chapter models the relationship between specific linguistic features and listener perceptions.
\end{description}
