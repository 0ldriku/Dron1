...
This study involved 36 participants (25 female and 11 male), recruited from a university population in Japan. All were Chinese international students learning Japanese. The participants' length of stay in Japan averaged 3.33 years ($SD = 2.78$, $range = 0.20\text{--}12.00$), and their majors were categorized as science ($n=19$) or arts and social sciences ($n=17$). All had passed the Japanese-Language Proficiency Test (JLPT) with N1 (highest level; $n=26$), N2 (second-highest level; $n=9$), or N3 (third-highest level; $n=1$). Furthermore, the participants were distributed across different academic levels: four undergraduates (one in year 2 and three in year 4), 19 master's students (15 in year 1 and four in year 2), and 13 doctoral students (four in year 1, six in year 2, and three in year 3). All participants provided informed consent, and the study was approved by the Human Subjects Research Ethics Review Committee of the Institute of Science Tokyo (Approval No. A24166). Two participants were excluded due to technical issues, yielding a final sample of N=34 for eye-movement (Study 4) and pupillometry analyses (Study 5). All 36 participants were retained for Study 3.
...
\subsubsection{Linguistic Complexity Manipulation}
...
The syntactic complexity manipulation was characterized using multiple metrics, detailed in Table \ref{tab:metrics}. Sentence length, expressed as the mean number of tokens per sentence, averaged 46.07--57.23 and 24.44--32.35 tokens in the complex and simple versions, respectively ($SD = 16.63\text{--}23.11$ versus $7.91\text{--}15.81$; maximum sentence length = 94--100 versus 43--87 tokens). Parse-tree\footnote{Parse-tree: Also known as a syntactic constituency tree, is a hierarchical data structure used in linguistics and computer science to represent the syntactic structure of a sentence according to a formal grammar. It visually breaks down a sentence into its constituent parts, such as noun phrases and verb phrases, showing their relationships down to the individual word level.} height, the depth of the dependency tree from the root to the deepest leaf, ranged from 7.04 to 8.82 nodes in the complex versions and 4.15 to 5.61 nodes in the simple versions. Mean dependency distance\footnote{Dependency distance: A measure of syntactic complexity used in sentence processing, defined as the linear distance (e.g., the number of intervening words) between two syntactically related words, or dependencies. Longer dependency distances are broadly associated with greater cognitive load, as they increase the memory demands required to connect the related elements.}, the average linear separation in tokens between syntactically linked heads and dependents, ranged from 2.99 to 3.27 tokens in the complex versions and 2.62 to 2.91 tokens in the simple versions. Constituent density followed the same pattern: noun phrases per sentence ranged from 12.29 to 14.23 in the complex versions and 7.04 to 10.30 in the simple versions, and particle--verb frames increased from 3--13 in the simple versions to 25--38 in the complex versions.

Lexical complexity metrics remained stable (Table~\ref{tab:metrics}). Document-level root type--token ratios,\footnote{Root type--token ratios: A measure of lexical diversity, calculated by dividing unique \textit{lemmatized} word types by the total \textit{lemmatized} tokens. This calculation required: \textit{tokenization} (the computational process of segmenting a continuous string of text into its component units, or 'tokens', e.g., words, particles, which is a non-trivial step in Japanese as it lacks explicit word delimiters); \textit{part-of-speech (POS) tagging} (the process of assigning a grammatical category, e.g., noun, verb, particle, to each token); and \textit{lemmatization} (the process of reducing an inflected word, e.g., \begin{CJK}{UTF8}{ipxm}食べた\end{CJK}, \textit{tabeta}, 'ate', to its base or dictionary form, e.g., \begin{CJK}{UTF8}{ipxm}食べる\end{CJK}, \textit{taberu}, 'to eat').} calculated for noun, verb, and prepositional phrase heads,\footnote{In this context, this refers to Japanese postpositional phrases, as the particle (e.g., \begin{CJK}{UTF8}{ipxm}に\end{CJK}, \begin{CJK}{UTF8}{ipxm}で\end{CJK}) follows the noun. The 'head' is the postposition (particle) itself, which governs the noun phrase.} varied by no more than 0.02 across versions, and the proportions of high-, mid-, and low-frequency tokens shifted by at most 3.47 percentage points.
...
\subsection{Presentation Modality and Screen Layout}




%-revision---reviewer 2 comment 3, reviewer 1 method comment 3
Each section was presented at $1920 \times 1080$ resolution as a complete, continuous unit on a single screen. Both text and video modalities shared an identical visual layout. A static, topic-relevant figure was displayed on the left side of the screen in both conditions. The right side differed depending on modality: for the text-based learning tasks, the corresponding text appeared on the right side of this figure, with the entire text for each section presented simultaneously on a single screen without requiring scrolling (see Figure \ref{fig:layoutt}); for the video-based learning tasks, the right side featured a video of a talking head delivering the narration (see Figure \ref{fig:layoutv}). A standard video player interface was presented at the bottom of the screen, featuring a timeline progress bar for unrestricted navigation and a central cluster of playback controls (e.g., play/pause, fast-forward, rewind). To isolate the modality effect and avoid confounding from redundant on-screen text, all videos were presented without captions. Word counts for all learning tasks and durations for video-based tasks are detailed in Table \ref{tab:word_count_duration}. The average narration speed across all video passages was approximately 310 words per minute, a rate consistent with Japanese news broadcasts. All narration audio was normalized to ensure consistent volume levels. 



\begin{figure*}[ht]
  \centering
  \fbox{\includegraphics[width=0.85\textwidth]{Chapters/c3fig/layout_t.png}}
  \caption{Layout of text modality}
  \label{fig:layoutt}
\end{figure*}

\begin{figure*}[ht]
  \centering
  \fbox{\includegraphics[width=0.85\textwidth]{Chapters/c3fig/layout_v.pdf}}
  \caption{Layout of video modality}
  \label{fig:layoutv}
\end{figure*}
...
To confirm that our manipulation of linguistic complexity was perceived by the participants as intended, we assessed perceived linguistic complexity after each learning task. The participants judged the overall linguistic complexity of the entire learning material by answering the following question: ``How complex did you find the language of the entire learning material you just studied?'' (1 = extremely simple, 7 = extremely complex). An example of the 7-point scale layout used in the experiment is shown in Figure \ref{fig:layouts}. All scales and questionnaires were presented in Chinese, the participants' first language, to minimize additional cognitive load, particularly for measures collected during the experimental tasks.



\begin{figure*}[ht]
  \centering
  \fbox{\includegraphics[width=0.85\textwidth]{Chapters/c3fig/layout_scale.png}}
  \caption{Example layout of the 7-point single-item scale}
  \label{fig:layouts}
\end{figure*}
...
Participants were positioned at a comfortable viewing distance (approximately 60 cm) from the monitor, with the eye-tracker mounted below the display and their head stabilized using a chin rest to ensure a consistent viewing angle throughout the experiment (see Figure \ref{fig:eyetracker}).


\begin{figure*}[ht]
  \centering
  \fbox{\includegraphics[width=0.8\textwidth]{Chapters/c3fig/eyetracker.png}}
...
\begin{table*}[!ht]
  ...
  \begin{threeparttable}
    \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}c *{2}{ccc}@{}}
      ...
      \midrule
      1        & Video & Simple  & Science 1
        & Text  & Complex & Science 2 \\
      2        & Video & Complex & History 1
        & Text  & Simple  & History 2 \\
      3 &Text &Complex &Science 2 & Video &Simple &Science 1 \\
      4 &Text &Simple &History 2 & Video &Complex &History 1\\
      \hline
    \end{tabular*}
    ...
    \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}c *{2}{ccc}@{}}
      ...
      2
        ...
        & Text  & Simple  & Science 2 \\
              3 &Text &Complex &History 2 & Video &Simple &History 1 \\
      4 &Text &Simple &Science 2 & Video &Complex &Science 1\\
      \bottomrule
    \end{tabular*}
    \begin{tablenotes}[flushleft]
      \footnotesize
...
\section{Analytical Roadmap for Studies 3, 4, and 5}

The data collected and processed in Experiment 2 are analyzed across the three subsequent chapters, with each study focusing on a distinct set of measures to provide a multi-level account of comprehension.

\begin{description}
    \item[Study 3 (\autoref{ch:study3})] Analyzes the subjective ratings (cognitive load and self-efficacy) and the comprehension accuracy scores. This study models how these outcomes are jointly shaped by linguistic complexity, domain, modality, and learner major.
    \item[Study 4 (\autoref{ch:study4})] Analyzes the aggregate eye-movement measures captured during the text modality tasks. This study examines temporal (e.g., fixation durations) and spatial (e.g., scan-path geometry) indices to trace the real-time allocation of visual attention.
    \item[Study 5 (\autoref{ch:study5})] Analyzes the continuous pupil diameter from both text and video modalities. This study models the temporal trajectories of physiological effort to understand how cognitive load fluctuates over time.
\end{description}
