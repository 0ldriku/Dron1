%************************************************
\chapter{Experiment 2 Methods (Studies 3, 4, and 5)}\label{ch:exp2-methods}
%************************************************
...
Experiment 2 shifts the focus from production to comprehension to address the academic learning research questions (RQ3, RQ4, and RQ5). Following the specific rationale developed in \autoref{subsec:op-comprehension}, intrinsic load is operationalized by manipulating linguistic complexity (simple vs. complex syntax). The design also varies presentation format (text vs. video) to test modality effects (\autoref{subsec:extraneous}) and contrasts content domain (history vs. science) against learners' academic major to test for expertise reversal effects (Section~\ref{subsec:boundaries}).

Verification of the load manipulation, guided by \autoref{sec:verification}, is operationalized using both objective pupillometry indices and subjective ratings. Pupillometry metrics (mean pupil diameter change and range dilation) provide physiological confirmation of processing demands across the entire task, while subjective ratings capture participants' experienced cognitive load following each content section. Participants also reported their self-efficacy after each section. In a methodological departure from Experiment 1, a secondary task was intentionally omitted. The inclusion of a dual-task paradigm would, by design, impose a competing, extraneous processing demand, compelling a division of attention. This would confound the measurement of processing effort originating solely from the comprehension materials. Beyond validation, eye-tracking and pupillometry serve as the primary dependent variables for the process-oriented analyses in Study 4 and Study 5. Interpretations of the main outcomes are therefore contingent upon both physiological and subjective measures confirming the efficacy of the linguistic complexity manipulation.

This common experimental design provides the data for three distinct studies. Study 3 analyzes the main and interactive effects of linguistic complexity, domain, and modality on subjective load, self-efficacy, and comprehension accuracy. Study 4 uses temporal and spatial eye-movement indices in the text modality to characterize how readers adjust their processing, testing whether domain expertise moderates the impact of linguistic complexity. Study 5 models continuous pupil dilation dynamics across both text and video as a time-resolved index of moment-to-moment effort. The remainder of this chapter details the participants, experimental design, materials and tasks, apparatus, instruments, procedure, and verification check.
...
\textbf{TODO: add the descriptive table of linguistic complexity}
