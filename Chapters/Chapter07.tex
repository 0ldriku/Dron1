...
Experiment 2 shifts the focus from production to comprehension to address the academic learning research questions. Following the specific rationale developed in \autoref{subsec:op-comprehension}, intrinsic load is operationalized by manipulating linguistic complexity (simple versus complex syntax). The design also varies presentation format (text versus video) to test modality effects (\autoref{subsec:extraneous}) and contrasts content domain (history versus science) against learners' academic major to test for expertise reversal effects (Section~\ref{subsec:boundaries}).
