...
The joint findings from Studies 1 and 2 lead to three practical consequences for L2 production tasks:
\begin{description}
    \item[Task design:] For tasks with high relational content, instructional scaffolding should help learners manage clause building and final-boundary timing, so that increased conceptual load does not undermine structural packaging.
    \item[Feedback and rubrics:] Feedback and assessment rubrics should prioritize final-boundary timing and clear morphosyntactic role marking (e.g. case particles), as these were the cues listeners relied on most when judging comprehensibility and fluency.
    \item[Score interpretation:] Decreases in fluency scores in high-complexity tasks should not be treated as simple deficits. When structural clarity and accuracy are preserved, slower speeds and shorter runs are expected, adaptive reallocations of resources.

\end{description}


These specific implications for speaking and listening are integrated with those from the comprehension experiments in \autoref{sec:practical}.
